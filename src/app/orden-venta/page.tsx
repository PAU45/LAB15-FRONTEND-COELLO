...
export default function OrdenVentaPage() {
  ...
  const fetchOrdenes = async () => {
    try {
      const res = await fetch("https://lab15-backend-coello.onrender.com/api/orden-venta");
    ...
    <div className="flex flex-col items-center justify-center min-h-screen bg-gradient-to-br from-blue-50 to-purple-100">
      <div className="bg-white/90 shadow-xl rounded-2xl p-8 w-full max-w-4xl border border-gray-200 mt-8">
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between mb-6 gap-4">
          <h1 className="text-3xl font-extrabold text-transparent bg-clip-text bg-gradient-to-r from-blue-600 to-purple-600 drop-shadow-lg">Lista de Órdenes de Venta</h1>
          <button
            ...
            onClick={() => router.push('/orden-venta/new')}
          >
            + Agregar Orden
          </button>
        </div>
        ...
        <div className="overflow-x-auto rounded-xl shadow">
          <table className="w-full text-center bg-white rounded-xl">
            ...
            <tbody>
              {ordenes.map((ord) => (
                <tr key={ord.NroOrdenVta} className="border-b hover:bg-blue-50 transition">
                  <td className="p-3">{ord.NroOrdenVta}</td>
                  <td className="p-3">{ord.fechaEmision?.slice(0, 10)}</td>
                  <td className="p-3">{ord.Motivo}</td>
                  <td className="p-3 flex flex-col sm:flex-row gap-2 justify-center">
                    <button
                      onClick={() => router.push(`/orden-venta/${ord.NroOrdenVta}/edit`)}
                      className="px-3 py-1 bg-yellow-400 text-white rounded-lg font-semibold hover:bg-yellow-500 shadow"
                    >
                      Editar
                    </button>
                    <button
                      onClick={() => eliminarOrden(ord.NroOrdenVta)}
