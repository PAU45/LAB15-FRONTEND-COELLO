    ...
    <div className="flex flex-col items-center justify-center min-h-screen bg-gradient-to-br from-blue-50 to-purple-100">
      <div className="bg-white/90 shadow-xl rounded-2xl p-8 w-full max-w-4xl border border-gray-200 mt-8">
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between mb-6 gap-4">
          <h1 className="text-3xl font-extrabold text-transparent bg-clip-text bg-gradient-to-r from-blue-600 to-purple-600 drop-shadow-lg">Lista de Detalles de Orden de Venta</h1>
          <button
            ...
            onClick={() => router.push('/detalle-orden-vta/new')}
          >
            + Agregar Detalle
          </button>
        </div>
        ...
        <div className="overflow-x-auto rounded-xl shadow">
          <table className="w-full text-sm text-gray-700 border border-gray-200 bg-white">
            ...
            <tbody>
              {detalles.map((det) => (
                <tr key={det.NroOrdenVta + '-' + det.CodMedicamento} className="hover:bg-purple-50 transition-colors">
                  <td className="p-3 border-b border-gray-100">{det.NroOrdenVta}</td>
                  <td className="p-3 border-b border-gray-100">{det.CodMedicamento}</td>
                  <td className="p-3 border-b border-gray-100">{det.cantidadRequerida}</td>
                  <td className="p-3 border-b border-gray-100 flex gap-2">
                    ...
                    <button
                      ...
                      onClick={() => eliminarDetalle(det.NroOrdenVta, det.CodMedicamento)}
                    >
                      Eliminar
                    </button>
                  </td>
              ...
              {detalles.length === 0 && (
                <tr>
                  <td colSpan={4} className="text-center py-6 text-gray-400">No hay detalles disponibles.</td>
                </tr>
              )}
            </tbody>
          </table>
